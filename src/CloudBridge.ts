...
filesApp.get(
  ...
  async function (req: express.Request, res: express.Response) {
    ...
    let path_cache = FILES_CACHE_DIR
      ? FILES_CACHE_DIR + "/" + id_robot.toString() + "/" + fname_cache
      : null;
    ...
    try {
      ...
      try {
        await fs.promises.access(
          FILES_CACHE_DIR + "/" + id_robot.toString(),
        ...
        try {
          ...
          await fs.promises.mkdir(FILES_CACHE_DIR + "/" + id_robot.toString(), {
            recursive: false,
          });
        } catch (err2) {
          if (err2.code != "EEXIST") {

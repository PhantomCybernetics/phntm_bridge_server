...
if (!ALL_SELECTED && !SELECTED_ROBOT_IDS.length && !SELECTED_SERVERS.length) {
    ...
    console.log('  --all Synchronize all robot ICE/TURN credentials to all ICE servers');
    console.log('  --robot ID_ROBOT Select robot to synchronize (e.g. 675aa6166400663224710eaf)');
    console.log('  --server HOSTNAME Select target server (e.g. ca.turn.phntm.io)');
    process.exit();
}
...
if (ALL_SELECTED) {
    ...
    if (SELECTED_ROBOT_IDS.length) {
        $d.log('Selected robot'+(SELECTED_ROBOT_IDS.length?'s':'')+': '+SELECTED_ROBOT_IDS.join(', '));
    }
    if (SELECTED_SERVERS.length) {
        $d.log('Selected server'+(SELECTED_SERVERS.length?'s':'')+': '+SELECTED_SERVERS.join(', '));
    }
}
...
ICE_SERVERS.forEach((one_server:string)=>{
    ...
    if ((ALL_SELECTED || SELECTED_SERVERS.indexOf(serverParts[1]) > -1) && filtered_servers.indexOf(serverParts[1]) == -1) {
...
if (!ALL_SELECTED) {
    SELECTED_SERVERS.forEach((one_selected)=>{
        if (filtered_servers.indexOf(one_selected) == -1) {
            $d.err('Server '+one_selected+' not found in config.jsonc, ignoring...');
        }
    });
}
...
mongoClient.connect().then(async (client:MongoClient) => {
    ...
    robots.forEach(async (robot:any)=>{
        ...
        if (!robot.ice_secret) {
            ...
            $d.log(' > No ice_secret found for '+robot._id.toString()+', setting newly generated');
            ...
            if (update_result.acknowledged) {
                ...
                $d.err(' > Update of '+robot._id.toString()+' not acknowledged; skipping');
                num_requests--;
                return exitIfAllDone(); //skip
            }
        }

        await Robot.SyncICECredentials(robot._id.toString(), robot.ice_secret, filtered_servers, SYNC_PORT, SYNC_SECRET, () =>{
            num_requests--;
